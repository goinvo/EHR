<header role="banner">
	<div class="container header">
		<div class="sixteen columns">
			<h1 class="title">Inspired EHRs: <span class="sm_title">Designing for Clinicians</span><!--<span class="version">v2, 3-24-2014</span>--></h1>
		</div>
	</div>

	<nav class="menu" role="navigation">
		<div class="container">
			<div class="sixteen columns">
				<ul>
					<li><a href="../">Table of Contents</a></li>
					<li><a href="../medication-list/" target="blank">Interactive Table</a></li>
					<li><a href="../timeline/" target="blank">Timeline</a></li>
					<li><a href="http://www.cs.umd.edu/hcil/sharp/twinlist/dev/indev/ipad/index.html?case=__DATASET_APPENDECTOMY__&amp;version=__VERSION_FULL__&amp;animate=__AUTO_ANIMATE_ON__" target="blank">Twinlist</a></li>
					<li><a href="../Inspired_EHRs_Designing_for_Clinicians.pdf" target="blank"  onclick="_gaq.push(['_trackEvent','Download','PDF',this.href]);">PDF</a></li>
					<li><a href="https://github.com/goinvo/EHR" target="blank">GitHub</a></li>
					<li><a href="./impact.php" >Impact</a></li>
					<li><a href="mailto:inspiredehrs@goinvo.com?Subject=EHR%20feedback" target="_top">Contact</a></li>
				</ul>
			</div>
		</div>
	</nav>
</header>
